...
const findHashedGeneration = (app: Cizn.Application, hash: string) => (generations: string[]) => {
  const foundGeneration = generations.find((g) =>
    g.includes(`${hash}-${<GenerationEnvironment> app.State.Environment}`)
  )
...
const assembleNewGeneration = (app: Cizn.Application, hash: string) => (generations: string[]) => {
  ...
  return {
    ...
    exists: false,
    path: `${generationNumber}-${hash}-${<GenerationEnvironment> app.State
      .Environment}`,
  }
}
 ...
 * @returns
 */
const getTargetGeneration =
  (app: Cizn.Application, hash: string) => (generations: string[]): Cizn.Application.State.Generation =>
    findHashedGeneration(app, hash)(generations) ||
    assembleNewGeneration(app, hash)(generations)
