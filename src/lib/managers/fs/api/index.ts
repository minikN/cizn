...
export type FSFileApi = {
  ...
  writeTemp: ({
    name, hash, ext, content,
  }: {name: string, hash: string, ext: string, content?: string}, errors?: {[key: string]: (...args: any) => any}) => Result<NonNullable<
...
export type FSPathApi = {
  ...
  getDirname: (a: string, errors?: {[key: string]: (...args: any) => any}) => Promise<Result<
    NonNullable<CiznError<'NO_PATH_GIVEN'>>, string
  >>

   getCwd: () => Result<CiznError<'NO_CWD_GIVEN'> | CiznError<'GENERIC_ERROR'>, string>
